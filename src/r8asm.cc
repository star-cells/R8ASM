...
std::map<std::string, int> datas = {{std::string("_UNIT_MAX"), UNIT_MAX}};

int read_opr(R8Operand arg, int default_opr = 1) {
    if (const std::string *operand = std::get_if<std::string>(&arg))
	return (datas[*operand]);
    else if (const int *operand = std::get_if<int>(&arg))
	return (*operand);
    return (default_opr);
}

void expand_xor(std::vector<rot8_bytecode> &tape, R8Operand arg) {
    int opr = read_opr(arg);
...
std::vector<rot8_bytecode> r8asm_tape_out(std::span<const R8Instruction> ops) {
	...
	if (const rot8_bytecode *bc = std::get_if<rot8_bytecode>(&it.op)) {
	    int repeat = read_opr(it.arg);
	    for (int i = 0; i < repeat; i++) tape.push_back(*bc);
